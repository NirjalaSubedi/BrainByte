  ...
  _setupInput() {
    ...
    this._handleGesture = (e) => {
      if (e.data?.type === 'BRAINBYTE_GESTURE') {
        const { gesture, x, y } = e.data;
        const gameCanvas = this.sys.game.canvas;
        const rect = gameCanvas.getBoundingClientRect();
        
        // Convert screen coordinates to canvas coordinates
        const canvasX = (x - rect.left) * (gameCanvas.width / rect.width);
        const canvasY = (y - rect.top) * (gameCanvas.height / rect.height);

        if (gesture === 'PINCH') {
          if (!this.aiming) {
            // pointerdown
            if (this.gameOver || this.playerDead) return;
            // Allow jumping via pinch
            if (Math.abs(canvasX - this.towerX) < 50 && Math.abs(canvasY - this.jumpBtn.y) < 25) {
              if (this.st >= 5 && !this.playerDead) {
                this.st -= 5;
                if (this.eb.onStChange) this.eb.onStChange(this.st);
                this.matter.applyForce(this.pRag.parts.torso.body, this.pRag.parts.torso.body.position, { x: 0, y: -0.1 });
            ...
            this.aiming = true;
            this.aimOrigin = { x: canvasX, y: canvasY };
          } else {
            // pointermove
    ...
    window.addEventListener('message', this._handleGesture);
    
    // Cleanup to prevent memory leaks when restarting scene
    this.events.once('shutdown', () => {
      window.removeEventListener('message', this._handleGesture);
    });
  }
